...
async function renderAdmin() {
  ...
  const [{ data: allTx }, { data: settings }, { data: customers }, { data: pendingWithdrawals }] = await Promise.all([
    ...
    sb.from("withdrawal_requests").select("*, profiles:user_id(full_name, phone)").eq("status", "pending").order("created_at", { ascending: false }),
  ]);

  app.innerHTML = topBar("Admin Dashboard") + `
    <div class="section-title">Pending Withdrawals (${(pendingWithdrawals || []).length})</div>
    <div class="card" style="padding:0;">
    ...
    <p style="padding:0 16px 8px;font-size:11px;color:var(--text-muted);">You send the money yourself (bank app / Paystack transfer) using these details, then tap "Mark Paid" here to close the request.</p>
