...
function providerOptionsHtml(list) {
  ...
  for (const s of list) {
    ...
    const name = pick(s, ["name", "service_name", "network", "provider_name", "network_name", "title", "label", "biller_name", "disco_name", "cable_name", "provider", "company", "description"], `Service ${id}`);
    if (seen.has(name)) continue; // VTUGATE lists the same provider multiple times (once per plan family) — show it once
    seen.add(name);
